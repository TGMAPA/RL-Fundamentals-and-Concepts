...
## Ejemplo: Actualización de parámetros con Policy Gradient (Gaussian PDF, Online)
...
Actualización de parámetros para una política estocástica parametrizada por logits $θ$ y softmax.  

### Softmax & Probabilidades
Dada la política:

$$
\text{probs} = \text{softmax}(θ)
$$

### Gradiente online para acción tomada
Si se toma la acción $i$ con recompensa $r$:
...
$$
θ \leftarrow θ + \alpha \cdot (\text{grad} - \text{probs}) \cdot r
$$

### Interpretación
- La acción tomada se **refuerza**: $θ_i$ sube si $r$ es positivo.  
...
$$
θ_i \leftarrow θ_i + \lambda (1 - p(a_i)) \, \cdot r
$$

Para acciones no tomadas ($j \neq i$):

$$
θ_j \leftarrow θ_j - \lambda \, p(a_j) \,  \cdot r
$$
---

## Ejemplo: Actualización de parámetros softmax (Policy Gradient online)
